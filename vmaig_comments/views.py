...
class CommentControl(View):
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            logger.error(u'[CommentControl]此分析报告不存在:[%s]' % en_title)
        ...
        if text.startswith('@['):
            ...
            try:
                ...
                logger.error(u'[CommentControl]反馈引用错误:%s' % parent_str)
                return HttpResponse(u"请勿修改反馈代码！", status=403)

        if not text:
            logger.error(
                u'[CommentControl]当前用户输入空反馈:[{}]'.format(
                    user.username
                )
            )
            return HttpResponse(u"请输入反馈内容！", status=403)
        ...
        print_comment = u"<p>反馈：{}</p>".format(text)
        if parent:
